...
public class Evaluation {
	...
	public static void evalSMB(String aspProgram, String aspInstances)
	{
		...
		try {
			Timer t1 = new Timer();
			t1.startTimer();
			ProcessBuilder builder = new ProcessBuilder("./dlv.bin", "-brave", "-silent", aspProgram, aspInstances);
	...
	public static void evalSMC(String aspProgram, String aspInstances)
	{
		...
		try {
			Timer t1 = new Timer();
			t1.startTimer();
			Process pr = rt.exec("./dlv -cautious -silent "+aspProgram+" "+aspInstances+" > "
					+resFilesSMC);
			pr.waitFor();
			t1.endTimer();
			System.out.println("  Time(s): "+(t1.getTimeInterval()/1000.0));
	...
	public static void evalWFB(String aspProgram, String aspInstances)
	{
		...
		try {
			Timer t1 = new Timer();
			ProcessBuilder builder = new ProcessBuilder("./dlv.bin", "-wf", "-silent", aspProgram, aspInstances);
			builder.redirectOutput(new File("out.txt"));
			builder.redirectError(new File("err.txt"));
			Process pr = builder.start();
			t1.startTimer();
			...
			System.out.println("  Time(s): "+(t1.getTimeInterval()/1000.0));
			pr.destroy();
			//Process pr2 = rt.exec("grep -o "+  shape +" out.txt | wc -l");
			// pr2.waitFor();
			//BufferedReader in = new BufferedReader(new InputStreamReader(pr2.getInputStream()));
			//String line;
			//while ((line = in.readLine()) != null) {
	...
	public static void evalWFC(String aspProgram, String aspInstances)
	{
		...
		try {
			Timer t1 = new Timer();
			t1.startTimer();
			Process pr = rt.exec("./dlv.bin -wf -cautious -silent "+aspProgram+" "+aspInstances+" > "
					+resFilesWFC);
			pr.waitFor();
			t1.endTimer();
			System.out.println("  Time(s): "+(t1.getTimeInterval()/1000.0));
	...
	public static void main (String[] args)
	{
		...
		if (semantics.equals("sm")) {
		 evalSMB(aspProgram, aspInstances);
		} 
		else if (semantics.equals("wf")) {
		
		 evalWFB(aspProgram, aspInstances);
		}
		else 
			System.out.println("Err: wrong semantic option");
